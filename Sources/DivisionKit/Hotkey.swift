...
public struct Hotkey: Equatable, Sendable {
    ...
    /// `eventFlags` must be `CGEvent.flags` / `NSEvent.modifierFlags` bits, not Carbon `cmdKey`.
    public func matches(keyCode: UInt32, eventFlags: UInt64) -> Bool {
        key.carbonKeyCode == keyCode && modifiers == Modifier.fromEventFlags(eventFlags)
    }
}
...
extension Hotkey {
    public struct Modifier: OptionSet, Equatable, Sendable {
        ...
        /// Carbon `cmdKey` (1<<8) / `shiftKey` (1<<9) / `optionKey` (1<<11) / `controlKey` (1<<12).
        ...
        /// `NX_SHIFTMASK` / `NX_COMMANDMASK` and the matching `NSEvent.ModifierFlags` bits.
        public static let shiftEventFlag: UInt64 = 1 << 17
        public static let controlEventFlag: UInt64 = 1 << 18
        public static let optionEventFlag: UInt64 = 1 << 19
        public static let commandEventFlag: UInt64 = 1 << 20

        public var eventFlags: UInt64 {
            var flags: UInt64 = 0
            if contains(.shift) { flags |= Self.shiftEventFlag }
            if contains(.control) { flags |= Self.controlEventFlag }
            if contains(.option) { flags |= Self.optionEventFlag }
            if contains(.command) { flags |= Self.commandEventFlag }
            return flags
        }

        /// Device-independent bits shared by `NSEvent.modifierFlags` and `CGEvent.flags`.
        public static func fromEventFlags(_ flags: UInt64) -> Modifier {
            var modifiers: Modifier = []
            if flags & shiftEventFlag != 0 { modifiers.insert(.shift) }
            if flags & controlEventFlag != 0 { modifiers.insert(.control) }
            if flags & optionEventFlag != 0 { modifiers.insert(.option) }
            if flags & commandEventFlag != 0 { modifiers.insert(.command) }
            return modifiers
        }
    }
...
public enum HotkeyParser {
    ...
    /// Parses a modifier-only string such as `"cmd"` or `"cmd+opt"`.
    ...
    public static func parseKey(_ string: String) throws -> Hotkey.Key {
        let tokens = tokenize(string)
        guard tokens.count == 1 else { throw HotkeyParseError.unknownToken(string) }
        guard let key = Self.key(from: tokens[0]) else {
            throw HotkeyParseError.unknownToken(tokens[0])
        }
        return key
    }
